...
describe("curl installer", () => {
  ...
  it("preserves a job-level with: block when regenerating the canonical workflow", () => {
    const workflowPath = path.join(dir, ".github", "workflows", "robin.yml");
    fs.mkdirSync(path.dirname(workflowPath), { recursive: true });
    fs.writeFileSync(
      workflowPath,
      [
        ...
        "    with:",
        '      llm-temperature: "1" # Kimi accepts only 1',
        "    secrets:",
    ...
    expect(output).toContain("Preserved existing `with:` overrides");
    expect(canonical).toContain("review.yml@v2");
  ...
  it("is idempotent when the canonical workflow already carries a with: block", () => {
    const workflowPath = path.join(dir, ".github", "workflows", "robin.yml");
    fs.mkdirSync(path.dirname(workflowPath), { recursive: true });
    fs.writeFileSync(
      workflowPath,
    ...
    run();
    const regenerated = fs.readFileSync(workflowPath, "utf8");
    const secondOutput = run();

    expect(secondOutput).toContain("is already current");
    expect(fs.readFileSync(workflowPath, "utf8")).toBe(regenerated);
  ...
  it("does not carry a step-level with: block from a legacy direct-action workflow", () => {
    ...
    expect(canonical).toBe(fs.readFileSync(TEMPLATE, "utf8"));
    expect(canonical).not.toContain("with:");
  });
